...
@Component
@Slf4j
public class FmStreamHandler extends BinaryWebSocketHandler {
  ...
  public void handleBinaryMessage(WebSocketSession session,
    ...
    try {
      StreamMessage streamMessage =
          StreamMessage.parseFrom(message.getPayload());
      switch (streamMessage.getMessageCase()) {
      case JOIN:
        log.info("Join Message for stream {} form session {}",
                 streamMessage.getJoin().getStreamId(), session.getId());
