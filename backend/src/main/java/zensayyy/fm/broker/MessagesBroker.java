...
public class MessagesBroker {
  ...
  public MessagesBroker() {
    streamUpdates =
        new ConcurrentHashMap<StreamJoin, LinkedList<StreamUpdate>>();
    streamSessions =
        new ConcurrentHashMap<StreamJoin, HashSet<WebSocketSession>>();
  }

  public void addSession(StreamJoin streamJoin, WebSocketSession session) {
    if (!streamUpdates.containsKey(streamJoin) ||
        !streamSessions.containsKey(streamJoin)) {
    ...
    log.info("Added session {} to stream {}", session.getId(),
             streamJoin.getStreamId());
    ...
    if (updates != null) {
      ...
      try {
        session.sendMessage(
            new BinaryMessage(StreamMessage.newBuilder()
                                  .setAction(StreamAction.FRESH)
                                  .setUpdateSummary(updateSummary)
                                  .build()
                                  .toByteArray()));
      } catch (IOException e) {
        log.error("Broker failed to send messages to {}: {}", session.getId(),
                  e);
      }
    }
  }
  ...
  public void addStreamUpdate(StreamJoin streamJoin,
    ...
    if (sessions != null) {
      for (WebSocketSession session : sessions) {
        try {
          session.sendMessage(new BinaryMessage(StreamMessage.newBuilder()
